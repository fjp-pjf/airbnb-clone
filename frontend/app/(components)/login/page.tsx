...
const Login = () => {
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    console.log(data);
    const isMatchEmail = emailRegex.test(data.email);
    const isMatchPassword = passwordRegex.test(data.password);

    if (!isMatchEmail || !isMatchPassword) {
      return toast(
        `Invalid Credentails. Please provide correct ${
          !isMatchEmail ? "Email" : "Password"
        }`
      );
    }
  };
